...
import com.example.crudmn.entity.*;
...
import com.example.crudmn.utils.ModelMapper;
...
//    private static final Logger logger = LoggerFactory.getLogger(PollService.class);
    ...
    public PagedResponse<PollResponse> getAllPolls(UserDetailsImpl currentUser, int page, int size) {
        ...
        Map<Long, Userpoll> creatorMap = getPollCreatorMap(polls.getContent());
    ...
    public PagedResponse<PollResponse> getPollsVotedBy(String username, UserDetailsImpl currentUser, int page, int size) {
        ...
        Map<Long, Long> choiceVoteCountMap = getChoiceVoteCountMap(pollIds);
        Map<Long, Long> pollUserVoteMap = getPollUserVoteMap(currentUser, pollIds);
        Map<Long, Userpoll> creatorMap = getPollCreatorMap(polls);
    ...
    public PollResponse getPollById(Long pollId, UserDetailsImpl currentUser) {
        ...
        Userpoll creator = userRepository.findById(poll.getCreatedBy())
                .orElseThrow(() -> new ResourceNotFoundException("User", "id", poll.getCreatedBy()));

        // Retrieve vote done by logged in user
        Vote userVote = null;
        if(currentUser != null) {
            userVote = voteRepository.findByUserIdAndPollId(creator.getId(), pollId);
        }
    ...
    public PollResponse castVoteAndGetUpdatedPoll(Long pollId, VoteRequest voteRequest, UserDetailsImpl currentUser) {
        ...
        Userpoll user = userRepository.findByUsername(currentUser.getUsername()).get();
                //getOne(currentUser.getId());
    ...
    private void validatePageNumberAndSize(int page, int size) {
        ...
        if(size > AppContsants.MAX_PAGE_SIZE) {
            throw new BadRequestException("Page size must not be greater than " + AppContsants.MAX_PAGE_SIZE);
        }
    }
    ...
    private Map<Long, Long> getPollUserVoteMap(UserDetailsImpl currentUser, List<Long> pollIds) {
        ...
        Userpoll _me = userRepository.findByUsername(currentUser.getUsername()).get();
        if(currentUser != null) {
            List<Vote> userVotes = voteRepository.findByUserIdAndPollIdIn(_me.getId(), pollIds);
    ...
    Map<Long, Userpoll> getPollCreatorMap(List<Poll> polls) {
        ...
        List<Userpoll> creators = userRepository.findByIdIn(creatorIds);
        Map<Long, Userpoll> creatorMap = creators.stream()
                .collect(Collectors.toMap(Userpoll::getId, Function.identity()));
